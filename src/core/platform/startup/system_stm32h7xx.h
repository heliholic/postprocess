...
/*
 * What every STM32H7 part in this tree provides out of its reset path. The
...
/*
 * Bring-up, in the order the reset handler in startup_<part>.s calls it.
 * Everything between systemPreInit() and SystemInit() is the sections: ITCM,
 * .data, .bss and the placement pools, which the reset path brings up itself.
 *
 *   systemPreInit()  the core supply, the clock tree, the flash timing that
 *                    goes with it, and the SRAM clocks the pools are zeroed
 *                    under. Runs before .data is copied and .bss zeroed, so
 *                    nothing it reaches may touch static storage.
...
/*
 * sys_ck, in Hz: the PLL output the whole tree derives from. Every other clock
