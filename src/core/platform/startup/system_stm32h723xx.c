...
/*
 * STM32H723 bring-up: everything between reset and the C runtime.
 *
...
/*
 * Clock tree for this part: 520 MHz at voltage scale 0, the maximum with the
 * option bytes as delivered. The datasheet maximum of 550 MHz requires
 * CPUFREQ_BOOST, which disables the ECC on ITCM and DTCM — the memories the
 * control loop runs from.
 *
 ...
 * 2x PCLKx, which the /2 above makes 260 MHz on every APB.
 *
 * These are private to this file. Only sysClockHz() is exported; every other
 * clock is a fixed ratio of it that a driver derives.
 */
#define RFX_SYSCLK_HZ 520000000U
#define RFX_CPUCLK_HZ 520000000U
#define RFX_HCLK_HZ   260000000U
#define RFX_PCLK1_HZ  130000000U
#define RFX_PCLK2_HZ  130000000U
#define RFX_PCLK3_HZ  130000000U
#define RFX_PCLK4_HZ  130000000U
#define RFX_TIMCLK_HZ 260000000U
#define RFX_PLL1Q_HZ  130000000U
#define RFX_USBCLK_HZ 48000000U

/*
 * PLL1 settings per crystal frequency. M divides HSE down to the PLL input,
 * N multiplies that up to the 520 MHz VCO, and P and Q divide that down to
 * sys_ck and the pll1_q kernel clock. RGE declares the input range to
 * PLLCFGR: 1 is 2-4 MHz, 2 is 4-8 MHz.
 *
 * P is 1, which this part allows. The H743 PLL1 P divider takes even values
 * only.
 *
...
_Static_assert(PLL1_VCO_HZ >= 192000000U && PLL1_VCO_HZ <= 836000000U,
               "PLL1 VCO outside the wide range");
...
/* The vector table, from startup_stm32h723xx.s. */
...
static void memoryInit(void)
{
     ...
     * buffers there, so enable both blocks before anything reaches them.
...
{
    /* The core supply first: nothing below may raise the scale under Run*. */
    exitRun0Mode();
    ...
    /*
     * SYSCFG is needed from here on for the I/O compensation cell below. The
     * voltage scale needs nothing from it on this part: scale 0 is the zero
     * encoding of VOS, not the over-drive bit the H743 reaches it by.
     */
    ...
    /*
     * A 260 MHz AXI clock at scale 0 needs 3 wait states, RM0468 table 16.
     ...
     * and AHBs at 260 MHz against a 275 MHz limit and the APBs at 130 MHz
     * against 137.5 MHz, both of which must hold the instant sys_ck arrives.
     * The CPU alone takes it undivided.
     */
 ...
 * peripherals Device.
 *
 * No region is Write-Through. Cortex-M7 erratum 1259864 is unfixed before
 * r1p2, and this part's core revision is not established, so every region is
 * Write-Back or non-cacheable.
 */
static void mpuInit(void)
{
    static const ARM_MPU_Region_t regions[] = {
        ...
        },
        {
             ...
             * no cache maintenance. SRAM1 and SRAM2 are adjacent and 16 KB
             * each, so one region covers both.
             */
